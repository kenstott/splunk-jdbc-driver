...
public class JdbcMetaDataIntegrationTest extends BaseIntegrationTest {
    ...
    public void testTableDiscovery() throws SQLException {
        ...
        try (ResultSet tables = metaData.getTables(null, "splunk", "%", new String[]{"TABLE"})) {
            ...
            assertTrue("Should find 'authentication' table", foundTables.contains("authentication"));
        }
    }
    ...
    public void testColumnDiscovery() throws SQLException {
        ...
        try (ResultSet columns = metaData.getColumns(null, "splunk", "web", "%")) {
            System.out.println("Columns in 'web' table:");
            int count = 0;
            ...
            while (columns.next()) {
                ...
                assertEquals("Schema should be 'splunk'", "splunk", schemaName);
                assertEquals("Table should be 'web'", "web", tableName);
            ...
            assertTrue("Should find web columns", count > 0);
            // Common columns in web table - these may vary based on Splunk configuration
            // Just check that we found some columns
            assertTrue("Should find at least one column", foundColumns.size() > 0);
        }
    }
    ...
    public void testPrimaryKeyDiscovery() throws SQLException {
        ...
        // Check for primary keys in web table
        try (ResultSet primaryKeys = metaData.getPrimaryKeys(null, "splunk", "web")) {
            System.out.println("Primary keys in 'web' table:");
            int count = 0;
    ...
    public void testForeignKeyDiscovery() throws SQLException {
        ...
        // Check for foreign keys from web table
        try (ResultSet foreignKeys = metaData.getImportedKeys(null, "splunk", "web")) {
            System.out.println("Foreign keys in 'web' table:");
            int count = 0;
    ...
    public void testIndexDiscovery() throws SQLException {
        ...
        // Check for indexes on web table
        try (ResultSet indexes = metaData.getIndexInfo(null, "splunk", "web", false, false)) {
            System.out.println("Indexes on 'web' table:");
            int count = 0;
    ...
    public void testResultSetMetaData() throws SQLException {
        ...
        String query = "SELECT * FROM splunk.web LIMIT 1";
        ...
        try (Statement stmt = connection.createStatement();
            ...
            // Web table has many columns, just verify we got some
            assertTrue("Should have at least 1 column", columnCount >= 1);
        }
    }
}